...
export const meta = {
  ...
  date: '2026-07-31T18:45:00+08:00', // TODO: Update this
...
Bucharest has a metro, buses, trams and trolleys that get to all corners of the city. Google Maps works really well at telling you the quickest way to get.
...
You can always pay with contactless, however noten that not all machines accept contactless.

For example, in my experience on the trams, they have ticket machines at every door, however only one of these is accepts contactless payment, so if one of the machines doesn't, go and look for a more modern machine. At Anghel Saligny only one of the ticket gates accepts contactless - the others don't. I only knew that because the security guard directed me to the right one. Wouldn't have had a clue otherwise.
...
export const kingMihaiIParkImages = [
  ...
  },
  {
    ...
    caption: "If you're doing a slower run, you can really appreciate the beauty.",
  },
  {
...
Sara Green is a fully vegan restaurant, nearby to my most recent room in Bucharest, so I came here quite a lot.
...
Sometimes you can lie in bed and stream video without issue. Other times you can't even load a webpage, and that's more annoying than having no wifi there at all.

I was never able to locate the wifi router, but there's a locked cabinet in the storgae cupboard, so I'd assume it's in there, but that in itself is stupid, because if the router was out in the open it'd have two fewer walls to go through and would likely work when you're in bed still.

My only other grievance with this room, is that to open and look the front door, you have to use an app on your phone. There is no physical key.
...
Don't pull hard enough, the lock jams. Pull too hard, the lock jams. On the worst occasion, which was a very hot day so presumably the mechanism had expanded, it took me eight attempts before I finally got the front door open. I'm really not sure why that's a better system than just having a key.

Besides those two things, I loved this room.

The bed was really comfortable, there's loads of space for exercise in the living room, once you pull the rug out the way, the kitchen's really well equipped, it's the first time in my life that I've ever had a study, and each of the bedroom, the living room, and the study have their own air conditioners.
