...
The good news is there are some stores in Taipei that are very vegan-friendly, you just have to know where to look. For example, <ExternalLink url='https://maps.app.goo.gl/UepP1ZSueMgFW2oC9'>Noemi</ExternalLink> is a vegetarian supermarket located within Plantarium (a complex that also includes a vegan restaurant, vegan cooking classes, and a few other plant-based things). Noemi do sell a small number of items that include dairy or egg, however these are very clearly marked (in English) on the price labels, so you can know that if it doesn't say otherwise, then what you're buying is vegan. The one downside that I found to Noemi, is that once they're out-of-stock of something, they never seem to get it back again. As I'm a creature of habit I found that frustrating.
...
TODO: Add photo of Leezen (between Noemi and nuts) if you take one
...
Watsons also stock a vegan multivitamin with very pretty packaging, but not only is it more nutrient deficient than the Decathlon, it's three times the price.
...
export const veganMultivitaminImages = [
  {
    ...
    caption: "Leezen's own-brand multi is the best vegan multivitamin that I found in Taipei. This is the men's version, but they have an equivalent for women. 30 days costs NT$650.",
  },
  {
